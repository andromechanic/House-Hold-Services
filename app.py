...
class HouseholdRequest(db.Model):
    ...
    professional_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)  # Ensure this column is defined
    ...
    professional = db.relationship('User', back_populates='professional_requests', foreign_keys=[professional_id])  # Ensure this references professional_id 
...
def create_service():
    ...
    if request.method == 'POST':
        ...
        # Check for duplicate service_title
...
def user_login():
    if request.method == "POST":
        ...
        if user and check_password_hash(user.user_password, password):
            # Check if the user is flagged (approval_status is False)
